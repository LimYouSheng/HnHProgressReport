...
function makeSeed(){
 ...
 {id:'p2',code:'HH-026-002',name:'Community Sports Court',client:'Sample community client',location:'East district · Demo project',category:'Court resurfacing',start:before(9),end:after(25),status:'Active',assigned:['s1','s3'],description:'Demonstration project for renewing a community sports court. Record surface preparation, surfacing works and court markings. This is a fictional project, not a claim about H&H’s portfolio.',aiInstructions:'Identify the selected work stage. Do not state that work has passed inspection or is complete without owner confirmation.',cadence:'daily',interval:1,weekday:5,time:'16:30',minimum:3,hero:null,created:before(9)},
 {id:'p3',code:'HH-026-003',name:'Landscape Improvement Works',client:'Sample landscape client',location:'North district · Demo project',category:'Landscape works',start:before(4),end:after(34),status:'Active',assigned:['s4'],description:'Fictional demonstration project for landscape improvement works. Record site conditions, preparation and reinstatement. Do not invent dimensions or quantities.',aiInstructions:'Use concise, neutral captions. Request clarification when the work stage is uncertain.',cadence:'weekly',interval:7,weekday:3,time:'15:00',minimum:3,hero:null,created:before(4)}];
 const photos=SOURCE_TITLES.map((title,i)=>{const n=i+1;return {id:`ph${n}`,project:'p1',supervisor:i%2?'s2':'s1',source:n,src:ASSET.photos[n],fileName:`SACPS-photo-${String(n).padStart(2,'0')}.jpg`,stage:SOURCE_STAGE(n),title,note:title,date:before(Math.floor((35-i)/3)),uploadedAt:`${before(Math.floor((35-i)/3))}T${i%2?'15:42':'10:25'}:00`,status:n<=20?'approved':n<=32?'review':'queued',caption:n<=32?SAMPLE_CAPTIONS[i]:'',approvedBy:n<=20?'Owner (demo)':null,approvedAt:n<=20?`${before(1)}T16:00:00`:null,revision:1};});
 const tasks=[
 {id:'t1',project:'p1',type:'task',text:'Capture close-up photos of the turf joints from both ends.',due:today,repeat:'none',assigned:'s1',priority:'High',done:false},
 {id:'t2',project:'p1',type:'task',text:'Photograph infill distribution across the field.',due:after(1),repeat:'none',assigned:'s2',priority:'Normal',done:false},
 {id:'t3',project:'p1',type:'remark',text:'Include a wide site view and a close-up of each work stage. Keep descriptions factual; the owner will verify captions.',due:today,repeat:'weekly',assigned:'all',priority:'Normal',done:false},
 {id:'t4',project:'p1',type:'task',text:'Record the access protection before turf removal.',due:before(6),repeat:'none',assigned:'s1',priority:'Normal',done:true},
 {id:'t5',project:'p2',type:'task',text:'Upload the initial court-condition photographs.',due:today,repeat:'none',assigned:'s1',priority:'High',done:false},
 {id:'t6',project:'p3',type:'task',text:'Take a wide-angle record of the work area.',due:after(2),repeat:'weekly',assigned:'s4',priority:'Normal',done:false}];
