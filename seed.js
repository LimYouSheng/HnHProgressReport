...
function makeSeed(){
 ...
 const photos=SOURCE_TITLES.map((title,i)=>{const n=i+1;return {id:`ph${n}`,project:'p1',supervisor:i%2?'s2':'s1',source:n,src:ASSET.photos[n],fileName:`SACPS-photo-${String(n).padStart(2,'0')}.jpg`,stage:SOURCE_STAGE(n),title,date:before(Math.floor((35-i)/3)),uploadedAt:`${before(Math.floor((35-i)/3))}T${i%2?'15:42':'10:25'}:00`,status:n<=20?'approved':n<=32?'review':'queued',caption:n<=32?SAMPLE_CAPTIONS[i]:'',approvedBy:n<=20?'Owner':null,approvedAt:n<=20?`${before(1)}T16:00:00`:null,revision:1};});
 ...
 return {schema:3,today,supervisors,projects,photos,notifications,reports,activity};
}
