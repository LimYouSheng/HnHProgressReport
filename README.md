# H&H Resources — Field Reports, revision 3
...
## Revision 3 — mobile popups and photo captions

All dialogs now stay within the available viewport, with safe-area spacing, a scrollable content body and persistent header/footer controls. On narrow screens the project forms, supervisor assignments and photo-upload forms use one column. Date/time fields, selects, long titles, images and action buttons no longer force the popup wider than the screen. The overlay responds to visual-viewport changes; physical keyboard behaviour still requires device testing.

This includes Add/Edit Project, Upload Photos, photo preview/edit, Add/Edit Supervisor, caption change requests and approval, regeneration, deactivation and deletion confirmations. Desktop dialog widths remain capped instead of stretching across the page.

### One Caption per photo

There is no separate Site note field. Each selected upload has one optional **Caption** textarea. Supervisors can write it directly before submission or edit it later. Adding or removing another selected photo preserves the captions already entered.

Uploading no longer runs automatic caption generation. A supplied caption goes to owner review exactly as entered (apart from trimming outer whitespace). An empty caption stays empty and is queued for an owner to write or explicitly generate. Owner review fields and photo previews do not insert placeholder descriptions into empty captions.

Uploaded-photo cards show the full project name, project code, date, uploader, caption and Edit action. Both the project photo gallery and supervisor's My uploads screen use this layout. Owner review cards also show the project name and Edit action.

The photo editor contains project, work stage, photo record date and the single Caption field. Supervisors can edit only their own photos within assigned projects; owners can edit across projects. This filtering remains a demo-only access simulation, not server-side security. Changing an approved photo's caption or details clears its approval. Saving an unchanged photo does not. Existing saved-report snapshots retain their original captions.

Caption change requests return supervisors to the same photo editor. Their revised caption is submitted directly for review, not rewritten automatically from a separate note.

### Updating existing data

The IndexedDB database name is unchanged. Schema 1/2 workspaces migrate to schema 3 without resetting projects, supervisors, photographs or report archives. Existing captions, including deliberately empty captions, are retained. Any different legacy note is retained only in internal caption history; it is not exposed as another editable field. Existing report snapshots are not rewritten. This migration was tested against in-memory legacy records; full storage persistence across installed-app upgrades still requires device testing.

## Retained revision 2 interface
...
The service-worker cache version has been advanced to revision 3. An existing installed copy may need to be closed and reopened or refreshed after the updated worker activates. Hosting and installed-device update behaviour still require deployment testing.
...
- AI captions are generated only on owner request, from contextual templates, selected work stages and supplied sample captions. Images are not sent to an AI model, and no actual image-recognition service is connected.
...
See `TEST_NOTES.md` for the exact coverage. The revision-3 automated results cover popup geometry, the upload/caption/edit workflow, history navigation, form submissions, approval status, migration transformations and immutable report snapshots. The PDF renderer is unchanged; a new full PDF download/render regression was not part of the revision-3 test run. Actual PWA installation, hosted service-worker activation, real device-camera capture, local-database persistence across browser restarts, and browser-specific behaviour still need testing on the deployment environment and physical phones.
